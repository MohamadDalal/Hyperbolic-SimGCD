...
#SBATCH --output="logs/SimGCD_Aircraft_mp.log"
#SBATCH --job-name="SimGCD_Aircraft_mp"
...
outfile="logs/SimGCD_Aircraft_mp.out"
#errfile="logs/SimGCD_Aircraft_mp.err"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} venv/bin/python -m torch.distributed.run --master_port 12348 --nproc_per_node=8 train_mp.py \
