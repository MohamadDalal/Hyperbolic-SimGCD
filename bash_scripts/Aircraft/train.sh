...
#SBATCH --output="logs/SimGCD-Aircraft.log"
#SBATCH --job-name="SimGCD-Aircraft"
...
outfile="logs/SimGCD-Aircraft.out"

exp_id="SimGCD-Aircraft"
...
if [ $restarts -gt -1 ]; then
    ...
    srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} train.py \
            ...
            --avg_grad_norm 100.0 \
            --use_dinov2 \
            --checkpoint_path '/ceph/home/student.aau.dk/mdalal20/P10-project/Hyperbolic-SimGCD/dev_outputs/simgcd/log/SimGCD-Aircraft/checkpoints/model.pt'
else
    srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} train.py \
            --dataset_name 'aircraft' \
            --batch_size 128 \
